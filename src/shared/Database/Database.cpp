...
void PrintYouNeeDatabaseVersionExpectedByCore(const DBVersion& core_db_requirements)
{
...
bool Database::CheckDatabaseVersion(DatabaseTypes database)
{
    ...
    if (!result)
    {
        ...
        sLog.outErrorDb();
		PrintYouNeeDatabaseVersionExpectedByCore(core_db_requirements);
        sLog.outErrorDb();
    ...
    else if (current_db_content > core_db_requirements.minimal_expected_content)
    {
        ...
        sLog.outErrorDb("If so, check if revision.h file need to be patched in order to avoid these messages: https://github.com/mangoszero/server/blob/master/src/shared/revision.h.");
		db_vs_core_content_version_mismatch = true;
    };
