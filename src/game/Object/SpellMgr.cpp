...
void SpellMgr::CheckUsedSpells(char const* table)
{
    ...
    do
    {
        ...
        if (spell)
        {
            ...
            if (familyMask != UI64LIT(0xFFFFFFFFFFFFFFFF))
            {
                ...
                else
                {
                    ...
                    {
                        sLog.outError("Spell %u '%s' not fit to (" I64FMTD ") but used in %s.", spell, name.c_str(), familyMask, code.c_str());
                        continue;
                    }
                }
            }
        ...
        else
        {
            ...
            {
                if (effectIdx >= 0)
                    sLog.outError("Spells '%s' not found for family %i (" I64FMTD ") icon(%i) visual(%i) category(%i) effect%d(%i) aura%d(%i) but used in %s",
                                  name.c_str(), family, familyMask, spellIcon, spellVisual, category, effectIdx + 1, effectType, effectIdx + 1, auraType, code.c_str());
                else
                    sLog.outError("Spells '%s' not found for family %i (" I64FMTD ") icon(%i) visual(%i) category(%i) effect(%i) aura(%i) but used in %s",
