...
// Top of the uint32 player-GUID range, reserved as the AH bot's forged owner.
// The player-GUID allocator's overflow guard (ObjectGuid.cpp: m_nextGuid >=
// GetMaxCounter - 1 == 0xFFFFFFFE) trips AT this value, shuts the server down,
// and returns the top-of-range GUID (GetMaxCounter, NOT this reserved one), so a
// real character can never be branded with it. Reaching the boundary at all
// would take ~4.29 billion player GUIDs, so it is effectively unreachable.
